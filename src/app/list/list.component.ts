  ...
  template: `
    ...
    <ng-container *ngIf="itemList?.length; else ref">
      <ul class="list-group">
        <ng-container *ngFor="let item of itemList; let i = index">
          <ng-container
            *ngTemplateOutlet="link; context: { $implicit: i + 1, item: item }"
          ></ng-container>
        </ng-container>
      </ul>
    </ng-container>
    ...
    <ng-template #link let-item="item" let-i>
      ...
      >
        ...
        {{
          (i % 2 === 0 ? (item?.name | titlecase) : (item?.name | uppercase))
            | slice: 0:3
        }}
        - {{ item?.price | number: '1.0-2' | currency: 'USD':'code' }} . Created
        at: {{ item?.creationDate | formatDate }}
